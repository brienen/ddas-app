...
function RegistrationForm() {
  ...
  const [formLevering, setFormLevering] = useState(() => {
    const base = initialFormData.leveringen?.[0] ?? {};
    const org = base.aanleverende_organisatie ?? {};
    const hasItems = Array.isArray(org.contactpersonen) && org.contactpersonen.length > 0;
    const contactpersonen = hasItems
      ? org.contactpersonen
      : [{ naam: "", functietitel: "", email: "", telefoonnummer: "" }];
    return { ...base, aanleverende_organisatie: { ...org, contactpersonen } };
  });
  const [formTrajecten, setFormTrajecten] = useState(() => {
    const fromInit =
      initialFormData.leveringen?.[0]?.schuldhulptrajecten ?? [];
    return fromInit.length ? fromInit : [createEmptyTraject()];
  });
