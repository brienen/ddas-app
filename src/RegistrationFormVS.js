...
import originalSchema from './json_schema_Uitwisselmodel_VS.json';
...
// Schema's voor algemeen, levering en vroegsignalen
...
const schemaTrajecten = addDefsToNestedProperties(
  originalSchema.properties.leveringen.items.properties.vroegsignalen.items,
  sharedDefs
);
  ...
  leveringen: [
    {
      ...
      vroegsignalen: []  // Lege array voor vroegsignalen
    }
  ]
};
...
async function uploadJson(json, setFormAlgemeen, setFormLevering, setFormTrajecten, setCurrentTrajectIndex, bestandsnaam) {
  ...
  if (json.leveringen && json.leveringen.length > 0 && json.leveringen[0].vroegsignalen) {
    setFormTrajecten(json.leveringen[0].vroegsignalen);
    setCurrentTrajectIndex(0);
    console.log('Er zijn ' + json.leveringen[0].vroegsignalen.length + ' trajecten ingelezen');
    document.getElementById('uploadResultaat').innerHTML = 'Er zijn ' + json.leveringen[0].vroegsignalen.length + ' trajecten ingelezen uit ' + bestandsnaam;
  }
}
...
function RegistrationForm() {
  ...
  const [formTrajecten, setFormTrajecten] = useState(
    initialFormData.leveringen && initialFormData.leveringen.length > 0 &&
    initialFormData.leveringen[0].vroegsignalen
      ? initialFormData.leveringen[0].vroegsignalen
      : []
  );
  ...
  const goToNextTab = () => {
    const tabOrder = ["start", "general", "vroegsignalen", "export"];
          ...
          <Tabs activeKey={activeTab} onSelect={handleTabSelect} className="mb-3">
            ...
            <Tab eventKey="start" title="Start">
              ...
              <p>Handig bij het invoeren:
                <ul>
                  ...
                  <li>Bij de vroegsignalen start je met het eerste traject. Om een traject toe te voegen, klik je op "NIEUW TRAJECT" rechtsboven. Met de knoppen "vorig traject" en "volgend traject" kan je naar de andere trajecten navigeren.</li>
                </ul>
              </p>
              <hr />
            ...
            <Tab eventKey="general" title="Algemene gegevens">
              <div className="p-3 border rounded bg-light">
                ...
                <p>Over welke periode worden gegevens over vroegsignalen aangeleverd? Alle vroegsignalen die tijdens deze periode actief waren (dus gestart voor de einddatum levering en nog niet afgerond of afgerond na de startdatum levering) moeten meegenomen worden bij "vroegsignalen".<br />
            ...
            <Tab eventKey="vroegsignalen" title="vroegsignalen">
              <div className="p-3 border rounded bg-light">
                <h2>vroegsignalen</h2>
                <p>Vul de gegevens in met betrekking tot vroegsignalen die je wilt aanbieden. Vul steeds per client (of 2 clienten als ze gezamenlijk aansprakelijk zijn) de gegevens in van het schuldhulptraject wat hij/zij doorloopt, of heeft doorlopen. Zijn er meer cliënten maak dan nieuwe schuldhulptraject aan.</p>
                <div className="d-flex justify-content-between mt-3">
